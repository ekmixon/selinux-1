...
class Tests:
    ...
    def test_modules(self, sh):
        ...
        print(f"Transaction number: {trans_cnt}")
        print(f"Module list size: {mlist_size}")
        if self.verbose:
            print(f"List reference: {mlist}")
        ...
        for idx in range(mlist_size):
            ...
            print(f"Module name: {semanage.semanage_module_get_name(module)}")
    ...
    def test_seusers(self, sh):
        ...
        print(f"Query status (commit number): {status}")
        ...
        for seuser in slist:
            if self.verbose:
                print(f"seseuser reference: {seuser}")
            print(f"seuser name: {semanage.semanage_seuser_get_name(seuser)}")
            print(f"   seuser mls range: {semanage.semanage_seuser_get_mlsrange(seuser)}")
            print(f"   seuser sename: {semanage.semanage_seuser_get_sename(seuser)}")
    ...
    def test_users(self, sh):
        ...
        for user in ulist:
            if self.verbose:
                print(f"User reference: {user}")
            print(f"User name: {semanage.semanage_user_get_name(user)}")
            print(f"   User labeling prefix: {semanage.semanage_user_get_prefix(user)}")
            print(f"   User mls level: {semanage.semanage_user_get_mlslevel(user)}")
            print(f"   User mls range: {semanage.semanage_user_get_mlsrange(user)}")
            print(f"   User number of roles: {semanage.semanage_user_get_num_roles(user)}")
            print("   User roles: ")
    ...
    def test_ports(self, sh):
        ...
        for port in plist:
            ...
            range_str = str(low) if low == high else f"{str(low)}-{str(high)}"
            (rc, con_str) = semanage.semanage_context_to_string(sh, con)
            if rc < 0:
                con_str = ""
            print(f"Port: {range_str} {proto_str} Context: {con_str}")
    ...
    def test_fcontexts(self, sh):
        ...
        for fcon in flist:
            if self.verbose:
                print(f"File Context reference: {fcon}")
            ...
            if con := semanage.semanage_fcontext_get_con(fcon):
                (rc, con_str) = semanage.semanage_context_to_string(sh, con)
                if rc < 0:
                    con_str = ""
            else:
                con_str = "<<none>>"
            print(f"File Expr: {expr} [{type_str}] Context: {con_str}")
    ...
    def test_interfaces(self, sh):
        ...
        for iface in ilist:
            if self.verbose:
                print(f"Interface reference: {iface}")
            ...
            if rc < 0:
                if_con_str = ""
            print(
                f"Interface: {name} Context: {if_con_str} Message Context: {msg_con_str}"
            )
    ...
    def test_nodes(self, sh):
        ...
        for node in nlist:
            if self.verbose:
                print(f"Network node reference: {node}")
            ...
            print(f"Network Node: {addr}/{mask} ({proto_str}) Context: {con_str}")
    ...
    def test_writeuser(self, sh):
        ...
        if self.verbose:
            print(f"User name set: {semanage.semanage_user_get_name(user)}")
        ...
        print(f"Commit status (transaction number): {status}")
    ...
    def test_writeport(self, sh):
        ...
        high = semanage.semanage_port_get_high(port)
        if self.verbose:
            print(f"SEPort range set: {low}-{high}")

        semanage.semanage_port_set_proto(port, semanage.SEMANAGE_PROTO_TCP)
        if self.verbose:
            print(
                f"SEPort protocol set: {semanage.semanage_port_get_proto_str(semanage.SEMANAGE_PROTO_TCP)}"
            )
        ...
        if not exists:
            ...
            if self.verbose:
                print(f"Port range delete: {status}")
        else:
            ...
            if self.verbose:
                print(f"Port range modify: {status}")
    ...
    def test_writefcontext(self, sh):
        ...
        if self.verbose:
            print(f"SEFContext expr set: {semanage.semanage_fcontext_get_expr(fcon)}")

        semanage.semanage_fcontext_set_type(fcon, semanage.SEMANAGE_FCONTEXT_REG)
        if self.verbose:
            ftype = semanage.semanage_fcontext_get_type(fcon)
            print(f"SEFContext type set: {semanage.semanage_fcontext_get_type_str(ftype)}")
        ...
        if not exists:
            ...
            if self.verbose:
                print(f"File context delete: {status}")
        else:
            ...
            if self.verbose:
                print(f"File context modify: {status}")
    ...
    def test_writeinterface(self, sh):
        ...
        if self.verbose:
            print(f"SEIface name set: {semanage.semanage_iface_get_name(iface)}")
        ...
        if not exists:
            ...
            if self.verbose:
                print(f"Interface delete: {status}")
        else:
            ...
            if self.verbose:
                print(f"Interface modify: {status}")
    ...
    def test_writeboolean(self, sh):
        ...
        if status < 0:
            raise Error("Could not set name")
        if self.verbose:
            print(f"SEBool name set: {semanage.semanage_bool_get_name(pbool)}")

        semanage.semanage_bool_set_value(pbool, 0)
        if self.verbose:
            print(f"SEbool value set: {semanage.semanage_bool_get_value(pbool)}")

        (status, key) = semanage.semanage_bool_key_extract(sh, pbool)
        if status < 0:
            raise Error("Could not extract SEBool key")
        if self.verbose:
            print(f"SEBool key extracted: {key}")
